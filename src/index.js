  ...
  async open_booklog_jp() {
    ...
    if (settingJsonUrl != "") {
      const jsonImport = async (jsonUrl) => {
        ...
        //foreach JSON
        await jsonData.forEach(function (item, index) {
          ...
          if (item.category !== undefined) PageCategoryList.push("[[" + item.category + "]]\n");
          if (item.year !== undefined) PageYearList.push(item.year);//later sort
          if (item.author !== undefined) PageAuthorList.push("[[" + item.author + "]]\n");
          if (item.publisher !== undefined) {
            PagePublisherList.push(item.publisher);
            item.publisher = "出版社/" + item.publisher;
          }
          if (item.type !== undefined) PageTypeList.push("[[" + item.type + "]]\n");
